...
def predict_emotion(unseen_file):
    unseen_features = feature_extraction(unseen_file)
    unseen_features = np.expand_dims(unseen_features, axis=0)  # Expand dimensions to match the input shape
    unseen_features = np.expand_dims(unseen_features, axis=-1)  # Shape should be (1, 40, 1)
    # Predict the label
    y_prob_unseen = model.predict(unseen_features)
    y_pred_unseen = np.argmax(y_prob_unseen, axis=1)
    # Decode the predicted label
    predicted_label = enc.inverse_transform(np.eye(7)[y_pred_unseen])[0][0]
    return predicted_label


# Example usage
unseen_file = 'E:\Self Study\CodeAlpha\Task 2\s.wav'
predicted_emotion = predict_emotion(unseen_file)
print(f'The predicted emotion for the unseen audio file is: {predicted_emotion}')